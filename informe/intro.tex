...
modelos.
Luego de localizar los ojos, se redimensiona el recuadro de cada ojo a un
tamaño fijo de 6 x 10 píxeles al cual se aplica también normalización por
histograma (\textit{{histogram normalization}}).
La combinación del vector obtenido para cada ojo resulta en un vector de 120
dimensiones en cada \textit{{frame}}.
A través de un modelo de \textit{{ridge regression}}, de este espacio se pasa
luego al mundo dos dimensional de las coordenadas de la pantalla.
...
mirada del sujeto.
Sin embargo, al no contar con invarianza frente a movimientos de cabeza ni con
estructuras que limiten el movimiento del sujeto ocurrirá tarde o temprano una
descalibración del sistema.
Frente a esto una opción es realizar tareas cortas y calibrar repetidamente
\cite{{xu_2015_turker_gaze}}.
Esto puede resultar en un experimento de duración significativamente mayor,
además de ser tedioso e incluso innecesario pues podría ocurrir que la
herramienta no se haya descalibrado.
La opción explorada en este trabajo consiste en proveer una heurística que
decida cuándo se ha descalibrado el sistema.
En base a ella se podrá elegir cuándo realizar una recalibración.
No es obvio cómo implementar tal heurística, pues ocurre además que los modelos
de estimación de la mirada no suelen incluir un mecanismo que indique cuándo el
sistema ha pasado a estar descalibrado
\cite{{hansen_2009_eye_of_the_beholder}}.

Calibrar y mantener calibrado un sistema de \eyetracking web resulta entonces
un problema desafiante para el cual quedan varios interrogantes por responder.
